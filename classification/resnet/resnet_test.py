...
def ValidModel(imgs_path):
    ...
    for i in range(1, 23):
        ...
        imgs_acc.append(chromoi_acc / (chromoi_acc + chromoi_wrong))
    print("正确数：", acc, "错误数：", wrong, "总数：", acc + wrong)
    return imgs_acc, acc / (acc + wrong)


# function: 使用输出的模型对单张图像进行预测并输出预测类别
# params: 单张图像路径
# return: 图像预测结果（类别名）
def prediectImg(img_path):
    net = torch.load(
...
if __name__ == '__main__':
    imgs_acc, all_acc = ValidModel(
        "../../dataset/classification_dataset/valid/")
    print("this is all labels acc: ", imgs_acc)
