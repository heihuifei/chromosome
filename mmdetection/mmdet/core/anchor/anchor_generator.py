...
class AnchorGenerator:
        ...
        for i, base_size in enumerate(self.base_sizes):
            ...
            multi_level_base_anchors.append(
                self.gen_single_level_base_anchors( # 调用当前特征图的base_anchors
                    ...
                    scales=self.scales,             # 8 / 16 / 32 /64 /128
    ...
    def gen_single_level_base_anchors(self,
        ...
        w_ratios = 1 / h_ratios
        if self.scale_major:
            # 8 * ([3,1]) * 1 = 3个w, view(-1)是将tensor平铺成一维
            # w_ratios[:, None]是按照列(第1维)进行维数扩充
    ...
    # 在一层特征图上根据shiftx,shifty生成anchors
    ...
    # 生成base_anchor基础上，需要通过改变每个anchor的中心来生成网格anchors，即所有特征图上生成anchors
    ...
    # 对图像进行pad，生成anchor会在该区域也生成，但应该忽略，因此该函数给每个anchor赋予一个标签是否在有效像素上
