...
class BaseDenseHead(BaseModule, metaclass=ABCMeta):
    ...
    # 作用是计算rpn网络的损失, 并将损失以字典的形式返回
    @abstractmethod
    def loss(self, **kwargs):
        """Compute losses of the head."""
        pass

    # 作用是将网络预测的bbox结果转化为proposal, 为roi head提供候选区域
    ...
    def forward_train(self,
        ...
        """
        Args:
            x (list[Tensor]): Features from FPN. 经过FPN的featrues
            img_metas (list[dict]): Meta information of each image, e.g.,
                image size, scaling factor, etc. 一个batch的image的信息的list
            gt_bboxes (Tensor): Ground truth bboxes of the image,
                shape (num_gts, 4). 一个batch的gt bbox的list
            gt_labels (Tensor): Ground truth labels of each box,
                shape (num_gts,). 一个batch的gt labels的list
            gt_bboxes_ignore (Tensor): Ground truth bboxes to be
                ignored, shape (num_ignored_gts, 4). 一个batch忽略的gt bbox
            proposal_cfg (mmcv.Config): Test / postprocessing configuration,
                if None, test_cfg would be used 测试/后处理的配置

        Returns:
            tuple:
                losses: (dict[str, Tensor]): A dictionary of loss components. 一个loss字典
                proposal_list (list[Tensor]): Proposals of each image. 一个批次的图片的proposal列表
        """
