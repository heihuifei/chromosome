...
from mmdet.core import build_assigner, build_sampler, rbbox2roi, rbbox2result
from ..builder import (HEADS, build_head, build_roi_extractor, build_shared_head)


@HEADS.register_module()
class StandardRRoIHead(BaseModule, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        super(StandardRRoIHead, self).__init__(init_cfg)
    ...
    def forward_train(self,
        ...
        if self.with_bbox:
            bbox_results = self._bbox_forward_train(x, sampling_results,
                                                    gt_bboxes, gt_labels,
                                                    img_metas)
            losses.update(bbox_results['loss_rbbox'])
    ...
    def _bbox_forward_train(self, x, sampling_results, gt_bboxes, gt_labels,
        ...
        loss_rbbox = self.bbox_head.loss(bbox_results['cls_score'],
                                        bbox_results['bbox_pred'], rois,
                                        *bbox_targets)

        bbox_results.update(loss_rbbox=loss_rbbox)
        return bbox_results
